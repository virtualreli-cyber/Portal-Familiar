...
const STORES = ['Todos', 'Mercadona', 'Carrefour', 'Lidl', 'Frutería local', 'Panadería', 'Otro'];

export const ShoppingView: React.FC = () => {
  const { shoppingItems, addShoppingItem, toggleShoppingItem, deleteShoppingItem, clearCompletedShopping, customCategories } = useFamily();
  ...
  const [selectedStore, setSelectedStoreState] = useState<string>(() => 
    getUserPreferences(currentMember.id).shoppingStoreFilter
  );

  const isFilterActive = selectedCategory !== 'Todas' || selectedStore !== 'Todos';

  const setSelectedCategory = (cat: string) => {
    setSelectedCategoryState(cat);
    saveUserPreferences(currentMember.id, { shoppingCategoryFilter: cat });
  };
  const setSelectedStore = (st: string) => {
    setSelectedStoreState(st);
    saveUserPreferences(currentMember.id, { shoppingStoreFilter: st });
  };
  ...
  const [category, setCategory] = useState<CategoryShopping>('Frutas y Verduras');
  ...
  const filteredItems = shoppingItems.filter(item => {
    ...
    if (selectedStore !== 'Todos' && item.store !== selectedStore) return false;
    return true;
  });

  const completedCount = shoppingItems.filter(i => i.completed).length;
  const pendingCount = shoppingItems.filter(i => !i.completed).length;
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Organiza las compras de la casa por supermercado y categorías
          </p>
        </div>
      ...
      <form onSubmit={handleQuickAdd} className="bg-white p-2 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-2">
        <input
          type="text"
          placeholder="⚡ Añadir rápido (Ej: Leche, Manzanas, Pan...)"
          value={quickName}
      ...
      <div className={`p-4 rounded-2xl border transition shadow-xs space-y-3 ${
        ...
        {isFilterActive && (
          <div className="flex items-center justify-between text-xs font-bold text-emerald-900 border-b border-emerald-200 pb-2">
            <span>🛒 Filtros Activos en la Compra</span>
            <button
              onClick={() => {
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex-1">
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Supermercado / Tienda</label>
            <select
              value={selectedStore}
              onChange={(e) => setSelectedStore(e.target.value)}
              className={`w-full px-3 py-1.5 border rounded-xl text-xs font-semibold ${
                selectedStore !== 'Todos' ? 'bg-emerald-100 border-emerald-400 text-emerald-900 font-bold' : 'bg-slate-50 border-slate-200 text-slate-700'
              }`}
            >
                    ...
                    {item.store && (
                      <>
                        ...
                        <span className="flex items-center gap-1 font-semibold text-slate-700">
                          <Store className="w-3 h-3 text-slate-400" /> {item.store}
                        </span>
                      </>
                    )}
                    {item.estimatedPrice && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
            <div className="bg-emerald-600 text-white p-5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-5 h-5" />
                <h3 className="font-bold text-lg">Añadir Producto a la Compra</h3>
              </div>
              <button 
            ...
            <form onSubmit={handleDetailedAdd} className="p-5 overflow-y-auto space-y-4">
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Supermercado</label>
                  <input
                    type="text"
